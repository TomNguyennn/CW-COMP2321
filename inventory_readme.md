...
This project analyzes inventory data from a mid-sized manufacturing retailer with warehouses across Australia to identify items that are likely to become dead stock (items with no recent sales activity, no longer in marketing campaigns, and limited relevance to current customer needs).
...
- Enhance precision of warehouse facility storage planning
...
- **Financial Data:** Inventory Aging Report Unit Cost, Total Value
...
├── HoangKimNganNguyen_Assessment1.ipynb  # Detailed analysis notebook
...
3. Ensure the dataset is placed in the `Dataset/` folder

## 💻 Usage

Run the prediction script:
```bash
python dead_stock_prediction.py
```
...
3. **Feature Scaling:**
   - StandardScaler applied to normalize features (mean=0, variance=1)
...
- Slight overfitting in Random Forest (100% training accuracy) but still generalizes well
...
- Despite slight overfitting, demonstrates strong generalization

### Business Implications
1. Items with high "Over 2 Years Qty" should be prioritized for review
...
- **Course:** BUSA3020 - Advanced Analytics Techniques
- **Institution:** Business Analytics Program
- **Semester:** S2 2025
- **Assessment:** Programming Task 1 (30% of total grade)
...
Hoang Kim Ngan Nguyen

## 🙏 Acknowledgments

- Course instructors and teaching staff
- Dataset provided by BUSA3020 course materials
